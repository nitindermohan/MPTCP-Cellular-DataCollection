...
do
	...
	then
		...
		sleep $SLEEPTIMER
		# run again as there are now two Telewell cards
		sudo usb_modeswitch -c /etc/usb_modeswitch.conf
	...
	if ([ `ls -1 /dev/ttyUSB* | wc -l` > 4 ] && [ `ifconfig | grep wwan | wc -l` == 2 ]) #check if the USB modem has been registered
	then
		if [ "$CONNECT" == "0" ]
		then
	...
	if [ `ifconfig | grep ppp | wc -l` == 4 ] #check if the modem has been registered as PPP device
	then
		if [ "$CONNECTED" == "0" ]
		then
	...
	then
		...
		./home/pi/Documents/network-quality-modded.sh
